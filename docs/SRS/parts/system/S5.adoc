...
| <<FR-24>> | Replay and Validation | Low (Could Have) | Useful for debugging and validation but not essential initially.
|===
...
The MoSCoW technique was used to rank the requirements according to their significance for user experience, research ethics, practicality, and core functioning. Eye-tracking data collection, processing, and safe visualization depend on high-priority ("Must Have") features. While not required for the Rev 0 proof-of-concept, medium-priority ("Should Have") features enhance usability, maintainability, and flexibility. Stretch goals for further iterations are low-priority ("Could Have") elements (e.g., multi-device central camera operations, replay tooling).

One of the main issues was security. Classroom deployment would not be acceptable without encryption, authentication/authorization, and anonymization (<<FR-14>>, \\NFR-9>>, \\NFR-11>>, \\NFR-10>>), which are "Must Haves" because they safeguard participant data and guarantee adherence to institutional research ethics.
