  ...
  \multirow{11}{0.3\textwidth}{Behaviour-Hiding} 
...
\begin{itemize}
\item transition: Computes spatial mapping between gaze coordinates and screen coordinates via homography transformations.
...
\item privacyLog := privacyLog $\cup$ \{createPrivacyEvent("PseudonymGenerated", deviceID)\}
\end{itemize}
...
\item privacyLog := privacyLog $\cup$ \{createPrivacyEvent("AnonymizationRequested", deviceID)\}
\end{itemize}
...
\begin{itemize}
\item transition: privacyLog := privacyLog $\cup$ \{createPrivacyEvent("ConsentChecked", deviceID)\}
\item output: isConsented := consentMap[deviceID] (defaults to false if not present)
\item exception: None
\end{itemize}

\noindent logPrivacyEvent(event: PrivacyEvent):
\begin{itemize}
\item transition: privacyLog := privacyLog $\cup$ \{event\}
...
createPrivacyEvent: (String, DeviceID) $\rightarrow$ PrivacyEvent \\
\noindent \textit{Semantics:} Creates a structured event record for privacy logging.
...
Roles are provided by the calling context and are already authenticated outside this module. \\
Authorization policy is simplified to role and resource checks; more granular RBAC rules may be introduced later.

\subsubsection{Access Routine Semantics}

\noindent validateAccess(userRole: Role, resource: ResourceID):
\begin{itemize}
\item transition: accessLog := accessLog $\cup$ \{createAccessRecord(userRole, resource)\}
\item output: permissionStatus := (userRole is permitted to access resource)
\item exception: Raises AuthError if the userRole is invalid or the resource identifier is malformed.
\end{itemize}

\noindent logAccess(event: AccessEvent):
\begin{itemize}
\item transition: accessLog := accessLog $\cup$ \{event\}
\item output: status := logged
\item exception: None
\end{itemize}
...
\noindent \textit{Semantics:} Creates an AccessEvent describing an attempted access by the given role to the given resource.
